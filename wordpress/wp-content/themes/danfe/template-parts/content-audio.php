			...
			<?php
			    if ( ! empty( $audio ) ) {
			        foreach ( $audio as $audio_html ) {
			            echo '<div class="danfe-audio-section">';
			                echo $audio_html;
			            echo '</div>';
			        }
			    }
			...
			<div class="post-header">
				    ...
				    <span class="post-category">
				    	<?php
	                       ...
	                       if ( ! empty( $categories ) ) {
	                          echo '<a href="'.esc_url( get_category_link( $categories[0]->term_id ) ).'" title="' . esc_html__('Audio Post Format', 'danfe') . '">'.esc_html( $categories[0]->name ).'</a>';
	                      }                                 
	                  ?>
				    </span>
			</div>
